...
InitialBinaries, mass_singles, mass_binaries, n_singles, n_binaries = \
     InitialBinaryTable.sampler('independent', final_kstar1, final_kstar2, binfrac_model=1.,
                                primary_model='kroupa93', ecc_model='sana12', porb_model='sana12',
...
rv.plot(kind='hist', bins=30, edgecolor='black', alpha=1, legend=None)
...
m1.plot(kind='hist', bins=25, edgecolor='black', alpha=1, legend=None)
plt.title('Mass 1 Distribution')
plt.xlabel('Mass(Solar Mass)')
plt.ylabel('Number')
#plt.xlim(0,2)
plt.savefig(path3 + 'Mass1_' + runID + '.png')
#plt.show()

m2.plot(kind='hist', bins=25, edgecolor='black', alpha=1, legend=None)
plt.title('Mass 2 Distribution')
plt.xlabel('Mass(Solar Mass)')
plt.ylabel('Number')
#plt.xlim(0,2)
plt.savefig(path3 + 'Mass2_' + runID + '.png')

q.plot(kind='hist', bins=25, edgecolor='black', alpha=1, legend=None)
plt.title('Mass Ratio')
plt.xlabel('Mass Ratio(M1/M2)')
plt.ylabel('Number')
plt.xlim(0,30)
plt.savefig(path3 + 'mass_ratio_' + runID + '.png')
#plt.show()

a.plot(kind='hist', bins=25, edgecolor='black', alpha=1, legend=None)
plt.title('Semi Major Axis')
plt.xlabel('a')
plt.ylabel('Number')
plt.savefig(path3 + 'semi_major_' + runID +'.png')
#plt.show()


#time = np.array(bpp.tphys[1])
#time = np.delete(time, -1)
#m1 = np.array(bpp.mass_1[1])
#m1 = np.delete(m1, -1)
#m2 = np.array(bpp.mass_2[1])
#m2 = np.delete(m2, -1)
#a = np.array(bpp.sep[1])
#a = np.delete(a, -1)

#Only difference I make is removing line 274
#fig, ax = plt.subplots()
#ax.plot(time, m1)
#ax.plot(time, m2)
#ax.plot(time, a)
...
#plt.savefig(path3 + 'Mass_evolution_' + runID +'.png')
